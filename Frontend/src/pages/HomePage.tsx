...
const HomePage = () => {
  ...
  const serverCount = useMemo(
    () => applications?.reduce((acc, app) => acc + app.servers.length, 0),
    [applications]
  );
  const sortedApplications = useMemo(() => {
    ...
    function filterApplications(app: Application) {
      return (
        ...
        app.servers.filter((server) => server.sourceIpAddress.includes(search))
          .length != 0 ||
        app.servers.filter((server) =>
          server.sourceHostname.toLowerCase().includes(search.toLowerCase())
        ).length != 0 ||
        app.servers.filter((server) =>
          server.destinationIpAddress.includes(search)
        ).length != 0 ||
        app.servers.filter((server) =>
          server.destinationHostname
            .toLowerCase()
            .includes(search.toLowerCase())
        ).length != 0 ||
        app.servers.filter((server) =>
          server.destinationPort.toString().includes(search)
        ).length != 0 ||
        app.servers.filter(
          (server) => server.enabled && search.toLowerCase().includes("yes")
        ).length != 0 ||
        app.servers.filter(
          (server) => !server.enabled && search.toLowerCase().includes("no")
        ).length != 0
      );
    }
    ...
    switch (sort.type) {
      ...
      case SortType.MODIFIED_AT:
        sorted = filtered.sort(
          (a, b) => Date.parse(a.modifiedAt) - Date.parse(b.modifiedAt)
        );
        break;
        ...
        <>
          <div className="flex justify-between w-full flex-col sm:flex-row">
            ...
            {user.role === UserRole.ADMIN && (
              <Button
                size="sm"
                className="h-min mt-4 sm:mt-0 whitespace-nowrap"
          ...
          <h3 className="text-xl font-medium mb-2">
            {user.role === UserRole.ADMIN ? "All" : "Your"} Applications
          </h3>
          <div className="flex gap-4 flex-col sm:flex-row">
            <TextInput
              placeholder="Search applications, server hostname, ip address..."
              icon={HiSearch}
            ...
            <div className="flex mb-6">
              ...
              <Dropdown
                ...
                renderTrigger={() => (
                  <div className="h-[42px]">
                    <Button
                      color="light"
                      className="flex items-center justify-between !border-l-0 !rounded-l-none whitespace-nowrap"
                    >
                      {sort.type}
              ...
              >
                {Object.values(SortType).map((v) => (
                  <Dropdown.Item
                    key={v}
                    onClick={() => setSort((s) => ({ ...s, type: v }))}
                    className={clsx(
                      sort.type === v && "bg-gray-200 dark:bg-gray-500"
                    )}
                  >
                    {v}
                  </Dropdown.Item>
